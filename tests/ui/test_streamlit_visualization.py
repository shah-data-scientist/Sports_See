...
def test_statistical_query_with_visualization(
    ...
    expect(messages).to_have_count(2)  # User message + Assistant message
...
def test_multiple_queries_conversation(
    ...
    expect(messages).to_have_count(2, timeout=10000)  # User + Assistant
    ...
    # Check that we have 4 messages (2 user + 2 assistant)
    messages = page.locator("[data-testid='stChatMessage']")
    expect(messages).to_have_count(4, timeout=10000)
