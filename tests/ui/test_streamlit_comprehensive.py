...
def test_initial_welcome_message(page: Page, streamlit_url: str):
    ...
    expect(messages.first).to_contain_text("Bonjour")
    expect(messages.first).to_contain_text("analyste IA")
...
def test_sidebar_present(page: Page, streamlit_url: str):
    ...
    expect(page.get_by_text("Paramètres")).to_be_visible()

    # Check for Feedback statistics section
    expect(page.get_by_text("Statistiques Feedback")).to_be_visible()
...
def test_negative_feedback_with_comment(
    ...
    comment_area = page.locator("textarea").filter(has_text="")
    expect(comment_area).to_be_visible(timeout=5000)

    # Enter comment
    # Find the textarea that's not the chat input
    textareas = page.locator("textarea").all()
    comment_textarea = None
    for textarea in textareas:
        placeholder = textarea.get_attribute("placeholder")
        if not placeholder or "question" not in placeholder.lower():
...
def test_clear_history_button(page: Page, streamlit_url: str):
    ...
    clear_button = page.get_by_text("🗑️ Effacer l'historique")
    ...
    messages = page.locator("[data-testid='stChatMessage']")
    expect(messages).to_have_count(1)
    expect(messages.first).to_contain_text("Historique effacé")
...
def test_sources_expander(page: Page, streamlit_url: str, screenshots_dir: Path):
    ...
    # Check for sources expander
    sources_expander = page.locator("summary").filter(has_text="Sources")
    expect(sources_expander).to_be_visible(timeout=10000)
...
def test_message_history_persists(page: Page, streamlit_url: str):
    ...
    messages = page.locator("[data-testid='stChatMessage']")
    # Should have: welcome + hello + response + thank you + response
    expect(messages.count()).to_be_greater_than_or_equal(4)
