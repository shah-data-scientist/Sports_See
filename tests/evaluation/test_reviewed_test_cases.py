...
class TestSQLReviewedTestCases:
    ...
    def test_sql_reviewed_count(self):
        """Test that SQL test cases count is within expected range (~48)."""
        from src.evaluation.sql_test_cases import SQL_TEST_CASES

        # Should have around 48 cases (allow some flexibility)
        assert 40 <= len(SQL_TEST_CASES) <= 60, f"Expected ~48 cases, got {len(SQL_TEST_CASES)}"
...
class TestHybridReviewedTestCases:
    ...
    def test_hybrid_reviewed_is_hybrid(self):
        ...
        assert len(weak_hybrid) < len(HYBRID_TEST_CASES) * 0.2, (
    ...
    def test_hybrid_reviewed_ground_truth(self):
        ...
        missing_ground_truth = []
        for tc in HYBRID_TEST_CASES:
            if not hasattr(tc, "ground_truth") or not tc.ground_truth:
                missing_ground_truth.append(tc.question[:60])

        assert (
            len(missing_ground_truth) == 0
        ), f"Cases missing ground_truth: {missing_ground_truth}"

    def test_hybrid_reviewed_count(self):
        """Test that Hybrid test cases count is within expected range (~18)."""
        from src.evaluation.hybrid_test_cases import HYBRID_TEST_CASES

        # Should have around 18 cases (allow some flexibility)
        assert 15 <= len(HYBRID_TEST_CASES) <= 25, (
            f"Expected ~18 cases, got {len(HYBRID_TEST_CASES)}"
        )
    ...
    def test_hybrid_reviewed_categories(self):
        ...
        # Should have both COMPLEX and NOISY categories
        assert TestCategory.COMPLEX in category_counts or "complex" in str(category_counts)
        assert TestCategory.NOISY in category_counts or "noisy" in str(category_counts)
...
class TestReviewedTestCasesComparison:
    ...
    def test_total_case_count(self):
        ...
        # Should have around 66 total cases (48 SQL + 18 Hybrid)
        assert 55 <= total_cases <= 85, f"Expected ~66 total cases, got {total_cases}"
