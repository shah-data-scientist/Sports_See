"""
FILE: evaluate_phase6.py
STATUS: Active
RESPONSIBILITY: Run full RAGAS evaluation with Phase 6 improvements (quality filter + content metadata)
...
CHECKPOINT_PATH = Path("evaluation_checkpoint_phase6.json")
BATCH_SIZE = 10
BATCH_COOLDOWN = 60
...
@logfire.instrument("generate_phase6_samples")
def generate_phase6_samples(
    chat_service: ChatService,
    prompt_template: str,
    prompt_variation_name: str,
) -> list[EvaluationSample]:
    """Generate samples using Phase 6 improvements (quality filter + content metadata).
...
@logfire.instrument("run_phase6_evaluation")
def run_phase6_evaluation(samples: list[EvaluationSample]) -> EvaluationReport:
    """Run RAGAS evaluation for Phase 6."""
...
def print_comparison_table(report: EvaluationReport, prompt_variation: str) -> None:
    ...
    print("\n" + "=" * 80)
    print(f"  PHASE 6 EVALUATION REPORT  |  Model: {report.model_used}")
...
def main(prompt_variation: str = "english_detailed") -> int:
    """Run Phase 6 evaluation with specified prompt template.
    ...
    logger.info("Initializing ChatService with Phase 6 improvements (quality filter + content metadata)...")
    ...
    try:
        samples = generate_phase6_samples(chat_service, prompt_template, prompt_variation)
    except Exception:
        logger.error("Sample generation interrupted. Re-run to continue.")
        return 1

    logger.info("Running RAGAS evaluation...")
    report = run_phase6_evaluation(samples)

    print_comparison_table(report, prompt_variation)

    # Save Phase 6 results
    report_path = Path("evaluation_results/ragas_phase6.json")
    ...
    logger.info(f"Phase 6 report saved to {report_path}")
...
if __name__ == "__main__":
    ...
    prompt_var = sys.argv[1] if len(sys.argv) > 1 else "english_detailed"
