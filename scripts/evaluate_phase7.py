...
@logfire.instrument("generate_phase7_samples")
def generate_samples(
    chat_service: ChatService,
    prompt_template: str,
    prompt_variation_name: str,
) -> list[EvaluationSample]:
    """Generate samples using Phase 7 improvements (winning prompt template).
...
@logfire.instrument("run_phase7_evaluation")
...
def main(prompt_variation: str = "balanced_french") -> int:
    """Run Phase 7 evaluation with specified prompt template.
    ...
    logger.info("Initializing ChatService with SQL enabled (Phase 7)...")
    ...
    report_path = Path("evaluation_results/ragas_phase7.json")
    report_path.parent.mkdir(parents=True, exist_ok=True)
    with report_path.open("w", encoding="utf-8") as f:
        json.dump(
            {
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "phase": "Phase 7 - Optimized Prompt Template",
    ...
    logger.info(f"Phase 7 report saved to {report_path}")
