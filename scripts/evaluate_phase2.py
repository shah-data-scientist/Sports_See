"""
FILE: evaluate_phase2.py
STATUS: Active
RESPONSIBILITY: Run RAGAS evaluation with Phase 2 SQL integration on existing test cases
...
CHECKPOINT_PATH = Path("evaluation_checkpoint_phase2.json")
BATCH_SIZE = 10
BATCH_COOLDOWN = 60
...
@logfire.instrument("generate_phase2_samples")
def generate_samples(chat_service: ChatService) -> list[EvaluationSample]:
    """Generate samples using Phase 2 hybrid RAG (SQL + Vector).
...
@logfire.instrument("run_phase2_evaluation")
...
def print_comparison_table(report: EvaluationReport) -> None:
    ...
    print("\n" + "=" * 80)
    print(f"  PHASE 2 EVALUATION REPORT  |  Model: {report.model_used}")
    print(f"  Samples: {report.sample_count}  |  SQL Integration: ENABLED")
    print("=" * 80)
...
def main() -> int:
    """Run Phase 2 evaluation on existing test cases.
    ...
    logger.info("Initializing ChatService with SQL enabled (Phase 2)...")
    chat_service = ChatService(enable_sql=True)  # KEY CHANGE: Enable SQL
    ...
    report_path = Path("evaluation_results/ragas_phase2.json")
    report_path.parent.mkdir(parents=True, exist_ok=True)
    with report_path.open("w", encoding="utf-8") as f:
        json.dump(
            {
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "phase": "Phase 2 - SQL Integration",
    ...
    logger.info(f"Phase 2 report saved to {report_path}")
