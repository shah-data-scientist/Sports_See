"""
FILE: evaluate_phase4.py
STATUS: Active
RESPONSIBILITY: Run RAGAS evaluation with Phase 4 improvements (prompt fix + classification + fallback)
...
CHECKPOINT_PATH = Path("evaluation_checkpoint_phase4.json")
BATCH_SIZE = 10
BATCH_COOLDOWN = 60
...
@logfire.instrument("generate_phase4_samples")
def generate_samples(chat_service: ChatService) -> list[EvaluationSample]:
    """Generate samples using Phase 4 improvements (prompt + classification + fallback).
...
@logfire.instrument("run_phase4_evaluation")
...
def print_comparison_table(report: EvaluationReport) -> None:
    ...
    print("\n" + "=" * 80)
    print(f"  PHASE 4 EVALUATION REPORT  |  Model: {report.model_used}")
    print(f"  Samples: {report.sample_count}  |  Improvements: Prompt + Classification + Fallback")
    print("=" * 80)
...
def main() -> int:
    """Run Phase 4 evaluation with prompt/classification improvements.
    ...
    logger.info("Initializing ChatService with SQL enabled (Phase 4 improvements)...")
    ...
    report_path = Path("evaluation_results/ragas_phase4.json")
    report_path.parent.mkdir(parents=True, exist_ok=True)
    with report_path.open("w", encoding="utf-8") as f:
        json.dump(
            {
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "phase": "Phase 4 - Prompt Engineering + Classification + SQL Fallback",
                "model": report.model_used,
                "sample_count": report.sample_count,
                "improvements": ["new_prompt", "conservative_classification", "sql_fallback"],
                "overall_scores": {
